...
#include "commons.h"
#include "kalman.hpp"
#include "ldpc_feedback.hpp"
#include "soft_combiner.hpp"
#include "whitening_processor.hpp"
               ...
               JS8::Event::Emitter emitEvent)
        {
            ...
            for (int ipass = 1; ipass <= 4 && totalLdpcPasses < m_maxLdpcPasses; ++ipass)
            {
                ...
                if (m_enableLdpcFeedback && totalLdpcPasses < m_maxLdpcPasses)
                {
                    ...
                    int confident = 0;
                    int uncertain = 0;
                    js8::refineLlrsWithLdpcFeedback(llrPrimary, cw, m_llrErasureThreshold, llrRefined, confident, uncertain);
...
: QObject(parent)
, m_semaphore(0)
, m_worker(new JS8::Worker(&m_semaphore))
{
...
void
JS8::Decoder::start(QThread::Priority priority)
{
