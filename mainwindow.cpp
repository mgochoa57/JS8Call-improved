  ...
  connect(m, &QNetworkAccessManager::finished, this, [this, alertOnUpToDate](QNetworkReply * reply){
    ...
    if(currentVersion < networkVersion){
        ...
        SelfDestructMessageBox * m = new SelfDestructMessageBox(60,
          ...
          QString("A new version (%1) of JS8Call is now available. Please see the <a href='https://github.com/JS8Call-improved/JS8Call-improved/releases'>GitHub Releases</a> for more details.").arg(content),
...
void MainWindow::setXIT(int audio_freq)
{
  if (m_transmitting && !m_config.tx_qsy_allowed ()) {
      qCWarning(mainwindow_js8) << "Ignoring change of audio freq to" << audio_freq << "as currently transmitting.";
      return;
  }
  ...
  // the audio actually transmitted is 1500 - 2160 Hz (TURBO has 160 Hz bandwith).
  // This way, the unwanted triple audio frequency possibly generated by audio distortions
  // is safely beyond the TX audio bandwidth of 3 kHz and will not result in transmission.
  // Also, the 1500 - 2160 Hz range should not be distorted or dampened by the TX's audio filter.
  ...
  if ((m_monitoring || m_transmitting)
      ...
      && m_config.split_mode())
    {
      ...
      qCDebug(mainwindow_js8) << "For incoming AF" << audio_freq
                              << "setting tx HF to" << m_freqTxNominal
                              << "and new AF to" << new_audio_frequency;
